...
function PopulaBanco() {
    ...
    useEffect(() => {
        const fetchJSON = async () => {
            try {
                console.log("cvhamadno back");
                const response = await api.get('/popula-banco'); // Chama a rota do backend
                setSuccess(true); // Define sucesso quando a resposta for recebida
                setLoading(false); // Define que o carregamento terminou
            } catch (error) {
                console.error("Erro ao buscar dados para popular o banco:", error);
                setError("Erro ao buscar dados para popular o banco."); // Armazena erro
                setLoading(false); // Define que o carregamento terminou
            }
        };
    ...
    if (success) {
        return <Navigate to="/" />; // Redireciona para a página de sucesso, por exemplo
    }

    return (
        <div>
            <h1>Detalhes do Livro</h1>
            <p>Dados populados com sucesso no banco de dados!</p>
        </div>
    );
}

export default PopulaBanco;
